...
from tests.kubernetes_mock_responses import (
    ...
    VALID_REG_SECRET, username, password, INVALID_SECRET
)
...
class CustomResourceTestCase(KubernetesTestCase):
    ...
    def test_update_statefulset_credentials(self):
        ...
        responses.add(
            'GET',
            '/api/v1/namespaces/default/secrets/internal-mirror',
            body=VALID_SECRET
        )
        responses.add(
            'GET',
            '/api/v1/namespaces/default/secrets/registry-mirror-hub',
            body=VALID_SECRET
        )
        responses.add(
            'PUT',
            '/api/v1/namespaces/default/secrets/registry-mirror-hub',
            body=VALID_REG_SECRET
        )
        ...
        self.assertEqual(
            "registry-mirror-hub",
            statefulset_json['spec']['template']
                            ['spec']['containers'][0]['env'][4]
                            ['valueFrom']['secretKeyRef']['name']
        )
        self.assertEqual(
            expected_url,
            base64.b64decode(secret_json['data']['url']).decode('utf-8')
        )
    ...
    def test_update_statefulset_create_url(self):
        ...
        self.assertEqual(
            None,
            statefulset_json['spec']['template']['spec']
                            ['containers'][0]['env'][4].get('value'))
        self.assertEqual(
            "registry-mirror-hub",
            statefulset_json['spec']['template']['spec']['containers']
                            [0]['env'][4]['valueFrom']['secretKeyRef']['name'])

    @responses.activate
    def test_update_statefulset_no_credentials(self):
        '''Should create the stateful set and fall back to upstream url if
    ...
    def test_update_statefulset_valid_old_secret(self):
        '''Should create the statefulset and  fall back to unuathed url
        if secret not there'''
        responses.add(
            'POST',
            '/apis/apps/v1beta1/namespaces/default/statefulsets',
    ...
    def test_update_statefulset_invalid_secret_valid_old_secret(self):
        '''Should create the statefulset and fall back to unauthed url
        if secret not valid'''
        responses.add(
            'POST',
            '/apis/apps/v1beta1/namespaces/default/statefulsets',
    ...
    def test_update_statefulset_invalid_secret_invalid_old_secret(self):
        '''Should create the statefulset and fall back to upstream url if
        neither secret is valid'''
        responses.add(
            'POST',
            '/apis/apps/v1beta1/namespaces/default/statefulsets',
            body=EMPTY_STATEFUL_SET
